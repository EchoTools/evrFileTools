...
type FileMetadata struct {
	TypeSymbol uint64 // File type identifier
	FileSymbol uint64 // File identifier
	Unk1       uint64 // Unknown - game launches with 0
	Unk2       uint64 // Unknown - game launches with 0
	AssetType  uint64 // Asset type identifier
}
...
func (m *Manifest) UnmarshalBinary(data []byte) error {
	...
	for i := 0; i < count; i++ {
		...
		m.Metadata[i].Unk1 = binary.LittleEndian.Uint64(data[offset+16:])
		m.Metadata[i].Unk2 = binary.LittleEndian.Uint64(data[offset+24:])
		m.Metadata[i].AssetType = binary.LittleEndian.Uint64(data[offset+32:])
...
func (m *Manifest) EncodeTo(buf []byte) {
	...
	for i := range m.Metadata {
		...
		binary.LittleEndian.PutUint64(buf[offset+16:], m.Metadata[i].Unk1)
		binary.LittleEndian.PutUint64(buf[offset+24:], m.Metadata[i].Unk2)
		binary.LittleEndian.PutUint64(buf[offset+32:], m.Metadata[i].AssetType)
		offset += FileMetadataSize
	}
